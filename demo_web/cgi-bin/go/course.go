...
var	connString =
	fmt.Sprintf("server=192.168.8.155;user id=SA;password=Helios123;port=1433;database=course")
...
func main() {
	...
	if err := cgi.Serve(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		class := r.FormValue("course")

		if ssn != "" && name != "" && class != "" {
			query := fmt.Sprintf("INSERT INTO class values ('%s', '%s', '%s');",
								ssn, name, class)
